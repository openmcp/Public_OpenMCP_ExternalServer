...
func main() {
	...
	if  len(os.Args) < 7 {
		...
		fmt.Println("Example : ./etcd-http-server --ip 10.0.3.12 --port 8090 --openmcp 10.0.3.20")
		return
	}
	for i, arg := range os.Args {
		if arg == "--ip" && i < len(os.Args){
			HTTPServer_IP = os.Args[i+1]
		} else if arg == "--port" && i < len(os.Args){
			HTTPServer_PORT = os.Args[i+1]
		} else if arg == "--openmcp" && i < len(os.Args){
			OpenMCP_IP = os.Args[i+1]
		}
	}
	if HTTPServer_IP == "" || HTTPServer_PORT == "" || OpenMCP_IP == "" {
		fmt.Println("Error Args!")
		fmt.Println("Example : ./etcd-http-server --ip 10.0.3.12 --port 8090 --openmcp 10.0.3.20")
		return
	}
